...
function openPetShop(){
  ...
  if(P.pet){
    ...
    cur.innerHTML='<div style="background:rgba(39,174,96,.1);border:1px solid #27ae60;border-radius:10px;padding:12px;margin-bottom:10px;display:flex;align-items:center;gap:10px"><div style="font-size:36px">'+P.pet.icon+'</div><div><div style="font-size:14px;font-weight:bold;color:#27ae60">'+esc(P.pet.name)+'</div><div style="font-size:11px;color:#666;margin-top:2px">'+(pd?esc(pd.desc):'')+'</div><div style="font-size:11px;color:#e74c3c;margin-top:4px;cursor:pointer" onclick="releasePet()">✕ Отпустить питомца</div></div></div>';
  } else {
  ...
  PETS_DEF.forEach(function(p){
    ...
    var btn=owned?'<div style="font-size:11px;color:#27ae60;font-weight:bold">✓ Активен</div>':'<button onclick="buyPet(\''+p.id+'\')" style="padding:6px 10px;border-radius:7px;border:none;background:var(--purple);color:#fff;font-size:11px;cursor:pointer">&#128176; '+p.price+'g</button>';
    div.innerHTML='<div style="font-size:28px">'+p.icon+'</div><div style="flex:1"><div style="font-size:13px;font-weight:bold;color:#d4c9a8">'+esc(p.name)+'</div><div style="font-size:10px;color:#666;margin-top:2px">'+esc(p.desc)+'</div></div>'+btn;
...
function buildClan(){
  ...
  if(!P.clan){
    ...
    content.innerHTML='<div style="background:var(--card);border-radius:12px;border:1px solid var(--border);padding:16px">'
      ...
      +(isLeader
        ?'<button onclick="disbandClan()" style="padding:8px 14px;border-radius:8px;border:none;background:#3d1a1a;color:#e74c3c;font-size:12px;cursor:pointer">💀 Распустить</button>':'<button onclick="leaveClan()" style="padding:8px 14px;border-radius:8px;border:none;background:#3d1a1a;color:#e74c3c;font-size:12px;cursor:pointer">🚪 Покинуть</button>')
...
function openMyHouse(){
  ...
  FURNITURE.forEach(function(f){
    ...
    div.innerHTML='<div style="font-size:24px">'+f.icon+'</div>'
      ...
      +(owned
        ?'<div style="font-size:11px;color:#27ae60;font-weight:bold">✓ Есть</div>':'<button onclick="buyFurniture(\''+f.id+'\')" style="padding:6px 12px;border-radius:7px;border:none;background:var(--purple);color:#fff;font-size:12px;cursor:pointer">💰 '+f.price+'g</button>');
    list.appendChild(div);
  });
  // Show active buffs
  ...
  if(!P.equipped||!Object.values(P.equipped).some(function(v){return v;})){
    ...
    Object.keys(P.equipped).forEach(function(slot){
      ...
      div.innerHTML='<div style="font-size:22px">'+item.i+'</div>'
        ...
        +'</div>'
        +(ench<ENCHANT_MAX
          ?'<button onclick="enchantItem(\''+id+'\')" style="padding:6px 10px;border-radius:7px;border:none;background:#5c2a0a;color:#e67e22;font-size:11px;cursor:pointer;border:1px solid #7a4010">⚒ '+ENCHANT_COST+'g</button>':'<div style="font-size:10px;color:#e67e22;font-weight:bold">МАКС</div>');
      list.appendChild(div);
    });
  }
...
function openQuestBoard(){
  ...
  QUESTS_DEF.forEach(function(q){
    ...
    div.innerHTML='<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">'
      ...
      +'</div>'
      +(done?'<div style="font-size:11px;color:#555">✓ Выполнено</div>':'<div style="height:6px;background:#1a1a2e;border-radius:3px;margin-bottom:6px"><div style="width:'+pct+'%;height:6px;background:'+(canClaim?'#27ae60':'#5c3d8f')+';border-radius:3px;transition:width .4s"></div></div>'
...
function renderDungeonRoom(){
  ...
  if(room.type==='combat'||room.type==='boss'){
    ...
    html='<div style="text-align:center;padding:14px">'
      ...
      +(room.cleared
        ?'<div style="font-size:12px;color:#27ae60;margin-bottom:10px">✓ Открыт</div><button onclick="dngAdvance()" style="padding:8px 20px;border-radius:8px;border:none;background:var(--purple);color:#fff;font-size:12px;cursor:pointer">→ Дальше</button>':'<button onclick="dngOpenTreasure()" style="padding:10px 24px;border-radius:9px;border:none;background:var(--gold);color:#000;font-size:13px;font-weight:bold;cursor:pointer">Открыть!</button>')
      +'</div>';
    ...
    html='<div style="text-align:center;padding:14px">'
      ...
      +(room.cleared
        ?'<div style="font-size:12px;color:#27ae60;margin-bottom:10px">✓ Отдохнул</div><button onclick="dngAdvance()" style="padding:8px 20px;border-radius:8px;border:none;background:var(--purple);color:#fff;font-size:12px;cursor:pointer">→ Дальше</button>':'<button onclick="dngRest()" style="padding:10px 24px;border-radius:9px;border:none;background:#27ae60;color:#fff;font-size:13px;font-weight:bold;cursor:pointer">Отдохнуть (+'+healAmt+' HP)</button>')
      +'</div>';
    ...
    html='<div style="text-align:center;padding:14px">'
      ...
      +(room.cleared
        ?'<div style="font-size:12px;color:#555;margin-bottom:10px">✓ Пройдена</div><button onclick="dngAdvance()" style="padding:8px 20px;border-radius:8px;border:none;background:var(--purple);color:#fff;font-size:12px;cursor:pointer">→ Дальше</button>':'<div style="font-size:12px;color:#666;margin-bottom:10px">Стрелковая ловушка (-'+trapDmg+' HP). Обезвредить? (50%)</div>'
...
function openItemModal(uid){
  ...
  inner.innerHTML=
    ...
    +(isEq
      ?'<button class="imbtn-eq" onclick="unequipItem(\''+entry.uid+'\',\''+entry.id+'\')">Снять</button>':'<button class="imbtn-eq" onclick="equipItem(\''+entry.uid+'\',\''+entry.id+'\')">Надеть</button>')
    +(thisIsEquipped&&sameIdCount===1
      ?'<button class="imbtn-sell" style="background:linear-gradient(135deg,#5c1a1a,#7c2020)" onclick="sellEquippedItem(\''+entry.uid+'\')">⚠ Продать (надет) '+item.sell+'g</button>':'<button class="imbtn-sell" onclick="sellItem(\''+entry.uid+'\')">Продать ('+item.sell+'g)</button>')
...
function renderRaidRoom(state){
  ...
  room.innerHTML=
    ...
    +(state.status==='waiting' ? '<div style="display:flex;gap:8px;margin-top:10px;align-items:center"><button class="raid-btn" onclick="startRaidNow()">Начать бой</button><span style="font-size:12px;color:#555">Код: <b style="color:var(--gold)">'+activeRaidId+'</b> — поделись с другом</span></div>' : state.win ? '<div style="color:var(--gold);font-weight:bold;font-size:15px;text-align:center;padding:10px">ПОБЕДА! +'+fmt(state.gold_gain)+' золота, +'+state.xp_gain+' XP!</div>' : state.lose ? '<div style="color:#e74c3c;font-weight:bold;font-size:15px;text-align:center;padding:10px">Поражение...</div>' : '<div style="display:flex;gap:8px;margin-top:10px"><button class="raid-btn" onclick="raidAtk(false)" '+(canAtk?'':'disabled')+'>Атака</button><button class="raid-btn raid-skill-btn" onclick="raidAtk(true)" '+(canAtk?'':'disabled')+'>Навык</button>'+(state.cooldown_left>100?'<span style="font-size:11px;color:#555;padding:9px 0">Перезарядка...</span>':'')+'</div>');
